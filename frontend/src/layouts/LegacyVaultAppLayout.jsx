/**
 * Legacy Vault Layout — 3-Layer Navigation (EOF Ecosystem UI/UX Canon)
 */
...
import {
  ...
  FiGrid, FiArchive, FiBook, FiFolder, FiSearch, FiSettings,
} from 'react-icons/fi';
import EOFTopNav from '../components/EOFTopNav';

const ACCENT        = '#A78BFA';
const ACCENT_SOFT   = 'rgba(167,139,250,0.10)';
const ACCENT_BORDER = 'rgba(167,139,250,0.28)';

const SIDEBAR_NAV = [
  { id: 'dashboard', label: 'Dashboard',  icon: FiGrid,    path: '/legacy-vault', exact: true },
  {
    id: 'archives', label: 'Archives', icon: FiArchive,
    children: [
      { label: 'All Archives',    path: '/legacy-vault', icon: '🏛️' },
      { label: 'Music Archives',  path: '/legacy-vault', icon: '🎵' },
      { label: 'Film Archives',   path: '/legacy-vault', icon: '🎬' },
      { label: 'Literature',      path: '/legacy-vault', icon: '📚' },
    ],
  },
  {
    id: 'collections', label: 'Collections', icon: FiFolder,
    children: [
      { label: 'My Collections', path: '/legacy-vault', icon: '📁' },
      { label: 'Curated',        path: '/legacy-vault', icon: '✨' },
      { label: 'Historical',     path: '/legacy-vault', icon: '⏳' },
    ],
  },
  {
    id: 'research', label: 'Research', icon: FiSearch,
    children: [
      { label: 'Akashic Records', path: '/akashic',     icon: '🔮' },
      { label: 'EOF Library',     path: '/',             icon: '📖' },
  ...
  },
  {
    ...
    children: [
      { label: 'Essays',         path: '/legacy-vault', icon: '✍️' },
      { label: 'Interviews',     path: '/legacy-vault', icon: '🎙️' },
      { label: 'Documentation',  path: '/legacy-vault', icon: '📝' },
    ],
  },
];

function VaultLogo({ collapsed }) {
  return (
    <div className={`flex items-center gap-3 select-none ${collapsed ? 'justify-center' : ''}`}>
      <div className="relative w-9 h-9 flex-shrink-0">
        <div className="w-full h-full rounded-xl flex items-center justify-center text-xl"
          style={{ background: 'linear-gradient(135deg, #A78BFA, #4C1D95)', boxShadow: '0 0 12px rgba(167,139,250,0.3)' }}>
          🏛️
        </div>
      ...
      {!collapsed && (
        <div className="leading-none min-w-0">
          <p className="font-serif text-sm text-white font-bold tracking-wide">Legacy Vault</p>
...
function DesktopSidebar({ collapsed, setCollapsed }) {
  const [openId, setOpenId] = useState('archives');
  return (
    <aside className={`flex-shrink-0 hidden lg:flex flex-col h-full overflow-hidden transition-all duration-200 ${collapsed ? 'w-14' : 'w-56'}`}
      ...
      <div className="px-2 pb-3 pt-2 border-t border-white/6 space-y-0.5">
        <NavLink to="/legacy-vault/widgets"
          className={({ isActive }) =>
            `flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs transition-all ${collapsed ? 'justify-center' : ''} ${
              isActive ? 'text-violet-300 bg-violet-900/20' : 'text-gray-600 hover:text-gray-400 hover:bg-white/5'
            }`
...
export default function LegacyVaultAppLayout() {
  const [drawerOpen, setDrawerOpen]           = useState(false);
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false);
  return (
    <div className="flex flex-col h-screen overflow-hidden" style={{ background: '#0d0d0d' }}>
      <EOFTopNav accentColor={ACCENT} systemName="Legacy Vault" systemEmoji="🏛️"/>
