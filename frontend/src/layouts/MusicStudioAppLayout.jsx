/**
 * Music Studio Layout — 3-Layer Navigation (EOF Ecosystem UI/UX Canon)
 */
...
import {
  ...
  FiGrid, FiMusic, FiMic, FiFolder, FiSend, FiSettings, FiHeadphones,
} from 'react-icons/fi';
...
const SIDEBAR_NAV = [
  { id: 'dashboard', label: 'Dashboard', icon: FiGrid,   path: '/music-studio', exact: true },
  {
    id: 'projects', label: 'Projects', icon: FiMusic,
    children: [
      { label: 'All Projects',   path: '/music-studio', icon: '🎵' },
      { label: 'In Progress',    path: '/music-studio', icon: '▶️' },
      { label: 'Mixing',         path: '/music-studio', icon: '🎚️' },
      { label: 'Mastered',       path: '/music-studio', icon: '✅' },
    ],
  },
  {
    id: 'recording', label: 'Recording', icon: FiMic,
    children: [
      { label: 'Studio Recorder', path: '/music-recorder', icon: '🎙️' },
      { label: 'Beat Builder',    path: '/music-studio',   icon: '🥁' },
      { label: 'Vocals',          path: '/music-studio',   icon: '🎤' },
      { label: 'Live Session',    path: '/music-studio',   icon: '🎸' },
    ],
  },
  {
    id: 'assets', label: 'Assets', icon: FiFolder,
    children: [
      { label: 'Samples',         path: '/music-studio',   icon: '📁' },
      { label: 'Beats',           path: '/music-studio',   icon: '🥁' },
      { label: 'Loops',           path: '/music-studio',   icon: '🔄' },
      { label: 'Plugins',         path: '/music-studio',   icon: '🔌' },
    ],
  },
  {
    ...
    children: [
      ...
      { label: 'Radio',           path: '/radio',          icon: '📻' },
      { label: 'Griot AI',        path: '/griot',          icon: '🌀' },
    ],
  },
];

function MusicLogo({ collapsed }) {
  return (
    <div className={`flex items-center gap-3 select-none ${collapsed ? 'justify-center' : ''}`}>
      <div className="relative w-9 h-9 flex-shrink-0">
        <div className="w-full h-full rounded-xl flex items-center justify-center text-xl"
          style={{ background: 'linear-gradient(135deg, #8B5CF6, #4C1D95)', boxShadow: '0 0 12px rgba(139,92,246,0.3)' }}>
          🎵
        </div>
      ...
      {!collapsed && (
        <div className="leading-none min-w-0">
          <p className="font-serif text-sm text-white font-bold tracking-wide">Music Studio</p>
  ...
  return (
    <aside className={`flex-shrink-0 hidden lg:flex flex-col h-full overflow-hidden transition-all duration-200 ${collapsed ? 'w-14' : 'w-56'}`}
      ...
      <div className="px-2 pb-3 pt-2 border-t border-white/6 space-y-0.5">
        <NavLink to="/music-studio/widgets"
          className={({ isActive }) =>
...
export default function MusicStudioAppLayout() {
  const [drawerOpen, setDrawerOpen]           = useState(false);
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false);
  return (
    <div className="flex flex-col h-screen overflow-hidden" style={{ background: '#0d0d0d' }}>
      <EOFTopNav accentColor={ACCENT} systemName="Music Studio" systemEmoji="🎵"/>
