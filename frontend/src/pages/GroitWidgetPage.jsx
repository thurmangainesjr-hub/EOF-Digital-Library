...
import GroitAppWidget, {
  SmallGroitWidget,
  MediumGroitWidget,
  LargeGroitWidget,
} from '../components/Widget/GroitAppWidget';

const SIZES = [
  { id: 'small',  label: 'Small',  dims: '155 × 155', desc: 'Active format + current project' },
  { id: 'medium', label: 'Medium', dims: '329 × 155', desc: 'Current project + recent adaptations' },
  { id: 'large',  label: 'Large',  dims: '329 × 345', desc: 'Full dashboard — all formats + stats' },
];

const MOCK_STATS = {
  activeFormat: { id: 'audiobook', icon: '🎧', label: 'Audiobook' },
  currentProject: { title: 'The Freedom Blueprint', type: 'Audiobook', icon: '🎧', status: 'In Progress' },
  recentProjects: [
    { title: 'The Freedom Blueprint', type: 'Audiobook',   icon: '🎧', status: 'In Progress' },
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden flex-shrink-0"
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 bg-black/30 border-b border-white/5">
        ...
        <span className="text-[10px] text-white/25 mx-auto">Griot AI</span>
      </div>
      <div className="absolute inset-0 opacity-5"
        style={{ backgroundImage: 'radial-gradient(circle, rgba(124,58,237,0.5) 1px, transparent 1px)', backgroundSize: '28px 28px' }} />
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function GroitWidgetPage() {
  const [activeSize, setActiveSize] = useState('medium');
  const [activeView, setActiveView] = useState('phone');

  function Preview() {
    if (activeSize === 'small') return <SmallGroitWidget stats={MOCK_STATS} />;
    if (activeSize === 'large') return <LargeGroitWidget stats={MOCK_STATS} />;
    return <MediumGroitWidget stats={MOCK_STATS} />;
  }
    ...
    <div className="min-h-full px-4 md:px-6 py-8 pb-16">
      ...
      <motion.div
        ...
        <div className="relative z-10 max-w-xl">
          ...
          <h1 className="font-serif text-3xl text-white font-bold mb-2">Griot AI Widgets</h1>
          <p className="text-sm text-gray-400 leading-relaxed">
            Add Griot AI to your home screen or desktop. See your active adaptation, current project, output format, and story count — at a glance.
          </p>
        </div>
      </motion.div>
        ...
        <div className="flex flex-wrap gap-6 items-end">
          {[
            ...
            <div key={size} className="flex flex-col items-center gap-3">
              <GroitAppWidget size={size} stats={MOCK_STATS} />
      ...
      <section className="mb-10">
        ...
        <p className="text-sm text-gray-500 mb-5">Install Griot AI as a native standalone app</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <InstallCard icon="🍎" title="iOS — Home Screen" accent="#7C3AED" steps={[
            'Open Griot AI in Safari',
            'Tap the Share button (square with arrow)',
            'Select "Add to Home Screen"',
            'Tap "Add" — widget appears instantly',
          ]} />
          <InstallCard icon="🤖" title="Android — Home Screen" accent="#10B981" steps={[
            'Open Griot AI in Chrome',
            'Tap the menu (three dots) top right',
            'Select "Add to Home Screen"',
            'Confirm — icon added to launcher',
          ]} />
          <InstallCard icon="🖥️" title="Windows Desktop" accent="#7C3AED" steps={[
            'Run create-griot-shortcut.ps1 from the project root',
            'Find "Griot AI.lnk" on your desktop',
            'Double-click to open in app mode',
            'Or click the install icon in Chrome\'s address bar',
          ]} />
        </div>
      </section>
      ...
      <div className="rounded-2xl border border-purple-600/25 p-6 flex flex-col md:flex-row md:items-center gap-4"
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-400">Opens Griot AI in standalone Chrome app mode — no browser chrome, pure creation focus.</p>
          <code className="inline-block mt-2 text-xs bg-black/40 border border-white/10 rounded px-3 py-1.5 text-green-400 font-mono">
            powershell -ExecutionPolicy Bypass -File create-griot-shortcut.ps1
          </code>
        </div>
        <Link to="/griot" className="flex items-center gap-2 px-5 py-3 rounded-xl text-sm font-medium flex-shrink-0 border border-purple-500/30 text-purple-300 hover:bg-purple-500/10 transition-colors">
          <FiDownload size={14} /> Back to Griot AI
        </Link>
      </div>
    </div>
  );
