...
const SIZES = [
  { id: 'small',  label: 'Small',  dims: '155 × 155', desc: 'Reading progress ring + current book' },
  { id: 'medium', label: 'Medium', dims: '329 × 155', desc: 'Book cover + progress + stats' },
  { id: 'large',  label: 'Large',  dims: '329 × 345', desc: 'Full dashboard — all books + streaks' },
];
  ...
  return (
    <div className="relative rounded-[44px] overflow-hidden flex-shrink-0"
      ...
      <div className="absolute inset-0"
        style={{ background: 'linear-gradient(160deg, #1a0f04 0%, #0d0d0d 40%, #0a0d0a 100%)' }} />
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden flex-shrink-0"
      style={{
        ...
        background: 'linear-gradient(135deg, #1a0f04 0%, #130d04 50%, #0d0d0a 100%)',
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 bg-black/30 border-b border-white/5">
        ...
        <span className="text-[10px] text-white/25 mx-auto">EOF Library</span>
      </div>
      <div className="absolute inset-0 opacity-5"
        style={{ backgroundImage: 'radial-gradient(circle, rgba(212,175,55,0.4) 1px, transparent 1px)', backgroundSize: '28px 28px' }} />
...
export default function LibraryWidgetPage() {
  ...
  function Preview() {
    const props = { book: MOCK_BOOK, stats: MOCK_STATS, recentBooks: MOCK_RECENT };
    if (activeSize === 'small') return <SmallWidget {...props} />;
    if (activeSize === 'large') return <LargeWidget {...props} />;
  ...
  return (
    <div className="min-h-full px-4 md:px-6 py-8 pb-16">
      ...
      <motion.div
        ...
        className="rounded-2xl border border-eof-gold/20 px-6 py-8 mb-8 relative overflow-hidden"
        style={{ background: 'linear-gradient(135deg, rgba(212,175,55,0.10), rgba(13,13,13,0.95))' }}>
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: 'radial-gradient(ellipse at 80% 50%, rgba(212,175,55,0.07), transparent 60%)' }} />
        <div className="relative z-10 max-w-xl">
          <div className="inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs mb-4 border"
            style={{ background: 'rgba(212,175,55,0.10)', borderColor: 'rgba(212,175,55,0.25)', color: '#D4AF37' }}>
            <FiGrid size={11} /> Widget Gallery — 3 Sizes
          </div>
          <h1 className="font-serif text-3xl text-white font-bold mb-2">Library Widgets</h1>
          <p className="text-sm text-gray-400 leading-relaxed">
            Add EOF Library to your home screen or desktop. See your current book, reading progress, streak, and recently added titles — at a glance.
          </p>
        </div>
      </motion.div>
              ...
              {SIZES.map(s => (
                <button key={s.id} onClick={() => setActiveSize(s.id)}
                  className={`w-full flex items-start gap-3 p-3 rounded-xl border text-left transition-all ${
                    activeSize === s.id
                      ? 'border-eof-gold/35 bg-eof-gold/8 text-white'
                      : 'border-white/8 bg-white/2 text-gray-500 hover:border-white/15'
                  }`}>
                  <div>
                    <p className={`text-sm font-medium ${activeSize === s.id ? 'text-eof-gold' : ''}`}>{s.label}</p>
                    <p className="text-xs text-gray-600 mt-0.5">{s.dims} — {s.desc}</p>
                  </div>
                  {activeSize === s.id && <span className="ml-auto w-2 h-2 rounded-full bg-eof-gold flex-shrink-0 mt-1" />}
                </button>
              ))}
            </div>
          </div>
              ...
              {[
                ...
                <button key={v.id} onClick={() => setActiveView(v.id)}
                  className={`flex-1 flex flex-col items-center gap-1 py-2.5 rounded-xl border text-xs transition-all ${
                    activeView === v.id
                      ? 'border-eof-gold/35 bg-eof-gold/8 text-eof-gold'
        ...
        <div className="flex flex-wrap gap-6 items-end">
          {[
            ...
            <div key={size} className="flex flex-col items-center gap-3">
              <AppWidget size={size} book={MOCK_BOOK} stats={MOCK_STATS} recentBooks={MOCK_RECENT} />
      ...
      <section className="mb-10">
        ...
        <p className="text-sm text-gray-500 mb-5">Install EOF Library as a native standalone app</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <InstallCard icon="🍎" title="iOS — Home Screen" accent="#D4AF37" steps={[
            'Open EOF Library in Safari',
            'Tap the Share button (square with arrow)',
            'Select "Add to Home Screen"',
            'Tap "Add" — widget appears instantly',
          ]} />
          <InstallCard icon="🤖" title="Android — Home Screen" accent="#10B981" steps={[
            'Open EOF Library in Chrome',
            'Tap the menu (three dots) top right',
            'Select "Add to Home Screen"',
            'Confirm — icon added to launcher',
          ]} />
          <InstallCard icon="🖥️" title="Windows Desktop" accent="#C0392B" steps={[
            'Run create-shortcut.ps1 from the project root',
            'Find "EOF Digital Library.lnk" on your desktop',
      ...
      {/* Shortcut CTA */}
      <div className="rounded-2xl border border-eof-gold/20 p-6 flex flex-col md:flex-row md:items-center gap-4"
        style={{ background: 'rgba(212,175,55,0.06)' }}>
        <div className="text-4xl flex-shrink-0">🖥️</div>
        <div className="flex-1">
          <h3 className="font-serif text-lg text-white font-bold mb-1">Windows Desktop Shortcut</h3>
          <p className="text-sm text-gray-400">Opens EOF Library in standalone Chrome app mode — no browser chrome, full focus.</p>
        ...
        <Link to="/" className="flex items-center gap-2 px-5 py-3 rounded-xl text-sm font-medium flex-shrink-0 border border-eof-gold/25 text-eof-gold hover:bg-eof-gold/8 transition-colors">
          <FiDownload size={14} /> Back to Library
        </Link>
      </div>
    </div>
  );
