...
import { FiChevronRight, FiGrid, FiSmartphone, FiMonitor, FiList } from 'react-icons/fi';
import ProjectsWidget, { SmallWidget, MediumWidget, LargeWidget } from '../components/Widget/ProjectsWidget';

const ACCENT = '#3B82F6';

const SIZES = [
  { id: 'small',  label: 'Small',  dims: '155 × 155', desc: 'Active count + system bars' },
  { id: 'medium', label: 'Medium', dims: '329 × 155', desc: 'Top projects + progress bars' },
  { id: 'large',  label: 'Large',  dims: '329 × 345', desc: 'All projects + system breakdown' },
];
...
function PhoneFrame({ size, children }) {
  ...
  return (
    <div className="relative rounded-[44px] overflow-hidden flex-shrink-0"
      ...
      <div className="absolute inset-0" style={{ background: 'linear-gradient(160deg, #080f1a 0%, #0d0d0d 40%, #0a0a0d 100%)' }} />
      <div className="relative z-10 flex flex-col items-center justify-center h-full gap-3 px-4 pt-8">
        <p className="text-[10px] font-semibold tracking-widest uppercase text-white/25 self-start px-2">Projects</p>
        {children}
      </div>
...
function DesktopFrame({ children }) {
  return (
    <div className="relative rounded-2xl overflow-hidden flex-shrink-0"
      style={{ width: 460, height: 300, background: 'linear-gradient(135deg, #080f1a 0%, #06101c 50%, #0d0d0a 100%)',
               border: '1px solid rgba(59,130,246,0.2)', boxShadow: '0 20px 60px rgba(0,0,0,0.6)' }}>
      <div className="flex items-center gap-2 px-4 py-2.5 bg-black/30 border-b border-white/5">
        ...
        <span className="text-[10px] text-white/25 mx-auto">Universal Projects</span>
      </div>
...
export default function ProjectsWidgetPage() {
  ...
  return (
    <div className="min-h-full px-4 md:px-6 py-8 pb-16">
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
        ...
        style={{ background: 'linear-gradient(135deg, rgba(59,130,246,0.10), rgba(13,13,13,0.95))', borderColor: 'rgba(59,130,246,0.2)' }}>
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: 'radial-gradient(ellipse at 80% 50%, rgba(59,130,246,0.07), transparent 60%)' }} />
        <div className="relative z-10 max-w-xl">
          <div className="inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs mb-4 border"
            style={{ background: 'rgba(59,130,246,0.10)', borderColor: 'rgba(59,130,246,0.25)', color: ACCENT }}>
            <FiGrid size={11} /> Widget Gallery — 3 Sizes
          </div>
          <h1 className="font-serif text-3xl text-white font-bold mb-2">📋 Projects Widgets</h1>
          <p className="text-sm text-gray-400 leading-relaxed">
            Add Universal Projects to your home screen. Track all your cross-system projects, tasks, progress, and system status — at a glance.
          </p>
          <div className="flex items-center gap-3 mt-4">
            <Link to="/projects" className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium border"
              style={{ background: 'rgba(59,130,246,0.10)', borderColor: 'rgba(59,130,246,0.25)', color: ACCENT }}>
              <FiList size={13}/> Open Projects
            </Link>
          </div>
        </div>
      </motion.div>
              ...
              {SIZES.map(s => (
                <button key={s.id} onClick={() => setActiveSize(s.id)}
                  className={`w-full flex items-start gap-3 p-3 rounded-xl border text-left transition-all ${
                    activeSize === s.id ? 'border-blue-500/35 bg-blue-500/8 text-white' : 'border-white/8 bg-white/2 text-gray-500 hover:border-white/15'
                  }`}>
                  <div>
                    <p className={`text-sm font-medium ${activeSize === s.id ? 'text-blue-400' : ''}`}>{s.label}</p>
                    <p className="text-xs text-gray-600 mt-0.5">{s.dims} — {s.desc}</p>
                  </div>
                  {activeSize === s.id && <span className="ml-auto w-2 h-2 rounded-full bg-blue-500 flex-shrink-0 mt-1" />}
                </button>
              ))}
            </div>
          </div>
              ...
              {[{ id: 'phone', icon: FiSmartphone, label: 'Phone' }, { id: 'desktop', icon: FiMonitor, label: 'Desktop' }, { id: 'bare', icon: FiGrid, label: 'Bare' }].map(v => (
                <button key={v.id} onClick={() => setActiveView(v.id)}
                  className={`flex-1 flex flex-col items-center gap-1 py-2.5 rounded-xl border text-xs transition-all ${
                    activeView === v.id ? 'border-blue-500/35 bg-blue-500/8 text-blue-400' : 'border-white/8 bg-white/2 text-gray-600 hover:text-gray-400'
                  }`}>
      ...
      <section className="mb-10">
        ...
        <p className="text-sm text-gray-500 mb-5">Install Universal Projects as a native app</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <InstallCard icon="🍎" title="iOS — Home Screen" accent={ACCENT} steps={['Open Projects in Safari', 'Tap Share → "Add to Home Screen"', 'Tap "Add" — icon appears instantly', 'Long-press to resize']} />
          <InstallCard icon="🤖" title="Android — Home Screen" accent="#10B981" steps={['Open Projects in Chrome', 'Tap menu (⋮) → "Add to Home Screen"', 'Confirm — icon added to launcher', 'Long-press to edit size']} />
          <InstallCard icon="🖥️" title="Windows Desktop" accent="#C0392B" steps={['Use the shortcut on your Desktop', 'Double-click "Projects.url"', 'Opens in Chrome app mode', 'Pin to taskbar for quick access']} />
        </div>
      </section>
      ...
      <div className="rounded-2xl border p-6 flex flex-col md:flex-row md:items-center gap-4"
        ...
        <div className="flex-1">
          <h3 className="font-serif text-lg text-white font-bold mb-1">Open Universal Projects</h3>
          <p className="text-sm text-gray-400">Manage every project across every system from one unified hub. Tasks, files, timelines, analytics.</p>
        </div>
        <Link to="/projects" className="flex items-center gap-2 px-5 py-3 rounded-xl text-sm font-medium flex-shrink-0 border"
          style={{ borderColor: 'rgba(59,130,246,0.25)', color: ACCENT }}>
          <FiList size={14}/> Projects Hub <FiChevronRight size={14}/>
        </Link>
      </div>
    </div>
  );
