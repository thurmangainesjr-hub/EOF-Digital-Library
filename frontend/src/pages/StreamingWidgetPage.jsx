...
const MOCK_STATS = {
  ...
  activeShows: 24,
  totalViewers: '18.7K',
  featured: { title: 'Black Excellence Documentary Series', channel: 'EOF Originals', icon: '📽️', viewers: '4.2K', ep: 'Ep 7 of 12' },
  channels: [
    { name: 'EOF Originals',   icon: '📽️', shows: 8,  color: '#6366F1', live: true  },
    { name: 'Culture & Arts',  icon: '🎨', shows: 6,  color: '#EC4899', live: true  },
    { name: 'Business & Wealth',icon: '💼', shows: 5,  color: '#16A34A', live: true  },
    { name: 'Health & Mind',   icon: '🧘', shows: 4,  color: '#0891B2', live: false },
    { name: 'Education',       icon: '🎓', shows: 3,  color: '#D4AF37', live: false },
    { name: 'Kids & Family',   icon: '👨‍👩‍👧', shows: 4,  color: '#F97316', live: false },
  ],
  categories: ['Drama', 'Documentary', 'Education', 'Talk Show', 'Music'],
};

export function SmallStreamingWidget({ stats = MOCK_STATS }) {
  return (
    <div className="rounded-3xl overflow-hidden flex-shrink-0 relative"
      ...
      <div className="relative z-10 h-full flex flex-col p-4 justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-black tracking-widest uppercase" style={{ color: ACCENT }}>Stream</span>
        </div>
        <div>
          <p className="text-[9px] text-gray-500 uppercase tracking-widest mb-0.5">Viewers Now</p>
          <p className="text-xl font-black text-white leading-none">{stats.totalViewers}</p>
          <p className="text-[10px] font-bold mt-1" style={{ color: ACCENT }}>{stats.activeShows} shows</p>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-[9px] text-gray-600">{stats.totalChannels} channels</span>
          <span className="text-[9px] px-2 py-0.5 rounded-full font-bold"
            style={{ background: 'rgba(99,102,241,0.15)', color: ACCENT }}>EOF+</span>
        </div>
      </div>
    </div>
  );
...
export function MediumStreamingWidget({ stats = MOCK_STATS }) {
  const feat = stats.featured;
  return (
    <div className="rounded-3xl overflow-hidden flex-shrink-0 relative"
        ...
        <div className="flex flex-col justify-between flex-shrink-0" style={{ width: 120 }}>
          <div>
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="text-[10px] font-black tracking-widest uppercase" style={{ color: ACCENT }}>Streaming</span>
            </div>
            <p className="text-[9px] text-gray-500 uppercase tracking-widest mb-0.5">Featured</p>
            <div className="flex items-center gap-1">
              <span>{feat.icon}</span>
              <p className="text-[10px] font-black text-white leading-tight">{feat.channel}</p>
            </div>
            <p className="text-[9px] text-gray-400 mt-0.5 leading-tight">{feat.ep}</p>
          </div>
          <div className="flex items-center gap-1">
            <span className="w-1 h-1 rounded-full animate-pulse" style={{ background: ACCENT }} />
            <span className="text-[9px] font-bold" style={{ color: ACCENT }}>{feat.viewers}</span>
          </div>
        </div>
          ...
          <div className="flex-1 space-y-1">
            {stats.channels.slice(0, 4).map(ch => (
              <div key={ch.name} className="flex items-center gap-2">
                ...
                <span className="text-[9px] text-gray-600">{ch.shows} shows</span>
              </div>
            ))}
          </div>
          <div className="flex items-center gap-2">
            <span className="text-[9px] text-gray-600">{stats.totalViewers} viewers</span>
            <span className="text-[9px] text-gray-600">·</span>
            <span className="text-[9px] text-gray-600">{stats.activeShows} shows</span>
          </div>
        </div>
      </div>
    </div>
...
export function LargeStreamingWidget({ stats = MOCK_STATS }) {
  const feat = stats.featured;
  return (
    <div className="rounded-3xl overflow-hidden flex-shrink-0 relative"
      ...
      <div className="relative z-10 h-full flex flex-col p-4 gap-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span>📺</span>
            <span className="text-xs font-black tracking-widest uppercase" style={{ color: ACCENT }}>EOF Streaming</span>
          </div>
          <span className="text-[9px] px-2 py-0.5 rounded-full border font-bold"
            style={{ color: ACCENT, borderColor: 'rgba(99,102,241,0.3)', background: 'rgba(99,102,241,0.08)' }}>{stats.totalViewers}</span>
        </div>
        {/* Featured */}
        <div className="rounded-2xl p-3 border" style={{ background: 'rgba(99,102,241,0.08)', borderColor: 'rgba(99,102,241,0.2)' }}>
          <p className="text-[9px] text-gray-500 uppercase tracking-widest mb-1">Featured Now</p>
          <div className="flex items-start gap-2">
            <span className="text-2xl flex-shrink-0">{feat.icon}</span>
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-black text-white leading-tight">{feat.title}</p>
              <p className="text-[9px] text-gray-400 mt-0.5">{feat.channel} · {feat.ep}</p>
            </div>
            <span className="text-[10px] font-bold flex-shrink-0" style={{ color: ACCENT }}>{feat.viewers}</span>
          </div>
        </div>
        {/* Stats row */}
        <div className="grid grid-cols-3 gap-2">
          {[['📺', stats.totalChannels, 'Channels'],['🎬', stats.activeShows, 'Shows'],['👥', stats.totalViewers, 'Viewers']].map(([icon, val, label]) => (
        ...
        <div className="flex-1">
          ...
          <div className="space-y-1.5">
            {stats.channels.map(ch => (
              <div key={ch.name} className="flex items-center gap-2 rounded-xl px-2 py-1.5"
                style={{ background: 'rgba(255,255,255,0.02)' }}>
                <span className="text-base flex-shrink-0">{ch.icon}</span>
                <span className="text-[10px] text-gray-200 font-medium flex-1 truncate">{ch.name}</span>
                <span className="text-[9px] text-gray-500">{ch.shows} shows</span>
                {ch.live && <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: ACCENT }} />}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex gap-1.5 flex-wrap pt-1 border-t border-white/6">
          {stats.categories.map(cat => (
            <span key={cat} className="text-[8px] px-2 py-0.5 rounded-full border font-medium"
              style={{ color: ACCENT, borderColor: 'rgba(99,102,241,0.25)', background: 'rgba(99,102,241,0.06)' }}>{cat}</span>
          ))}
        </div>
      </div>
    </div>
...
function StreamingAppWidget({ size = 'medium', stats = MOCK_STATS }) {
  if (size === 'small')  return <SmallStreamingWidget stats={stats} />;
  if (size === 'large')  return <LargeStreamingWidget stats={stats} />;
  return <MediumStreamingWidget stats={stats} />;
}
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden flex-shrink-0"
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 bg-black/30 border-b border-white/5">
        ...
        <span className="text-[10px] text-white/25 mx-auto">EOF Streaming</span>
      </div>
...
const SIZES = [
  { id: 'small',  label: 'Small',  dims: '155 × 155', desc: 'Viewer count + show total' },
  { id: 'medium', label: 'Medium', dims: '329 × 155', desc: 'Featured show + channel list' },
  { id: 'large',  label: 'Large',  dims: '329 × 345', desc: 'Full dashboard — all channels + categories' },
];

export function StreamingWidgetPage() {
  const [activeSize, setActiveSize] = useState('medium');
  const [activeView, setActiveView] = useState('phone');

  function Preview() {
    if (activeSize === 'small') return <SmallStreamingWidget />;
    if (activeSize === 'large') return <LargeStreamingWidget />;
    return <MediumStreamingWidget />;
  }
    ...
    <div className="min-h-full px-4 md:px-6 py-8 pb-16">
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="relative z-10 max-w-xl">
          ...
          <h1 className="font-serif text-3xl text-white font-bold mb-2">EOF Streaming Widgets</h1>
          <p className="text-sm text-gray-400 leading-relaxed">
            Add EOF Streaming to your home screen or desktop. See featured shows, channel activity, and viewer counts — at a glance.
          </p>
        </div>
      </motion.div>
      ...
      <section className="mb-10">
        ...
        <p className="text-sm text-gray-500 mb-5">Install EOF Streaming as a standalone app</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <InstallCard icon="🍎" title="iOS — Home Screen" accent={ACCENT} steps={['Open EOF Streaming in Safari','Tap the Share button','Select "Add to Home Screen"','Tap "Add" — widget appears instantly']} />
          <InstallCard icon="🤖" title="Android — Home Screen" accent={ACCENT} steps={['Open EOF Streaming in Chrome','Tap the menu (three dots)','Select "Add to Home Screen"','Confirm — icon added to launcher']} />
          <InstallCard icon="🖥️" title="Windows Desktop" accent={ACCENT} steps={['Run create-streaming-shortcut.ps1 from project root','Find "EOF Streaming.lnk" on your desktop','Double-click to open in app mode','Or click the install icon in Chrome\'s address bar']} />
        </div>
      </section>
      ...
      <div className="rounded-2xl border p-6 flex flex-col md:flex-row md:items-center gap-4"
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-400">Opens EOF Streaming in standalone Chrome app mode — pure cinematic focus, no browser clutter.</p>
          <code className="inline-block mt-2 text-xs bg-black/40 border border-white/10 rounded px-3 py-1.5 text-green-400 font-mono">
            powershell -ExecutionPolicy Bypass -File create-streaming-shortcut.ps1
          </code>
        </div>
        <Link to="/streaming" className="flex items-center gap-2 px-5 py-3 rounded-xl text-sm font-medium flex-shrink-0 border transition-colors"
          style={{ borderColor: 'rgba(99,102,241,0.3)', color: '#A5B4FC' }}>
          <FiDownload size={14} /> Back to Streaming
        </Link>
      </div>
    </div>
  );
...
export default StreamingWidgetPage;
