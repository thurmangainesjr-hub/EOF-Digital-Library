...
import UniversityAppWidget, {
  SmallUniversityWidget,
  MediumUniversityWidget,
  LargeUniversityWidget,
} from '../components/Widget/UniversityAppWidget';

const SIZES = [
  { id: 'small',  label: 'Small',  dims: '155 × 155', desc: 'Professor + school progress ring' },
  { id: 'medium', label: 'Medium', dims: '329 × 155', desc: 'Active session + school list' },
  { id: 'large',  label: 'Large',  dims: '329 × 345', desc: 'Full dashboard — all engines' },
];

const MOCK_STATS = {
  school: { id: 'writing', shortName: 'Writing', icon: '✍️', accent: '#D97706' },
  professor: { name: 'Professor Inkwell' },
  currentSchool: { id: 'writing', shortName: 'Writing', icon: '✍️', accent: '#D97706' },
  progress: 34,
  engine: 'Teaching',
  streak: 5,
  booksStudied: 6,
  hoursLearned: 18,
  completedCapstones: 2,
};
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0"
        style={{ background: 'linear-gradient(160deg, #1a0a2e 0%, #0d0d1a 40%, #0a0d1a 100%)' }} />
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden flex-shrink-0"
      style={{
        ...
        background: 'linear-gradient(135deg, #0f0820 0%, #130d1f 50%, #0a0d1f 100%)',
    ...
    >
      <div className="flex items-center gap-2 px-4 py-2.5 bg-black/30 border-b border-white/5">
        ...
        <span className="text-[10px] text-white/25 mx-auto">DIY University</span>
      </div>
      <div className="absolute inset-0 opacity-8"
        style={{ backgroundImage: 'radial-gradient(circle, rgba(124,58,237,0.3) 1px, transparent 1px)', backgroundSize: '28px 28px' }} />
      <div className="relative flex items-center justify-center h-full pb-8 pt-4">
        {children}
      </div>
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function UniversityWidgetPage() {
  const [activeSize, setActiveSize] = useState('medium');
  const [activeView, setActiveView] = useState('phone');

  function Preview() {
    if (activeSize === 'small')  return <SmallUniversityWidget stats={MOCK_STATS} />;
    if (activeSize === 'large')  return <LargeUniversityWidget stats={MOCK_STATS} />;
    return <MediumUniversityWidget stats={MOCK_STATS} />;
  }
  ...
  return (
    <div className="min-h-full px-4 md:px-6 py-8 pb-16">
      ...
      <motion.div
        ...
        className="rounded-2xl border border-purple-600/25 px-6 py-8 mb-8 relative overflow-hidden"
        style={{ background: 'linear-gradient(135deg, rgba(124,58,237,0.12), rgba(13,13,13,0.95))' }}
      >
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: 'radial-gradient(ellipse at 80% 50%, rgba(124,58,237,0.08), transparent 60%)' }} />
        <div className="relative z-10 max-w-xl">
          ...
          <h1 className="font-serif text-3xl text-white font-bold mb-2">University Widgets</h1>
          <p className="text-sm text-gray-400 leading-relaxed">
            Add DIY University to your home screen or desktop. See your active professor, school progress, streak, and all 5 professor engines — at a glance.
          </p>
        </div>
      </motion.div>
              ...
              {SIZES.map(s => (
                <button key={s.id} onClick={() => setActiveSize(s.id)}
                  className={`w-full flex items-start gap-3 p-3 rounded-xl border text-left transition-all ${
                    activeSize === s.id
                      ? 'border-purple-500/40 bg-purple-900/15 text-white'
              ...
              {[
                ...
                <button key={v.id} onClick={() => setActiveView(v.id)}
                  className={`flex-1 flex flex-col items-center gap-1 py-2.5 rounded-xl border text-xs transition-all ${
                    activeView === v.id
                      ? 'border-purple-500/40 bg-purple-900/15 text-purple-400'
                      : 'border-white/8 bg-white/2 text-gray-600 hover:text-gray-400'
                  }`}>
                  <v.icon size={14} />
                  {v.label}
                </button>
      ...
      <section className="mb-12">
        ...
        <div className="flex flex-wrap gap-6 items-end">
          {[
            ['small',  'Small — 155×155'],
            ['medium', 'Medium — 329×155'],
            ['large',  'Large — 329×345'],
          ].map(([size, label]) => (
            <div key={size} className="flex flex-col items-center gap-3">
              <UniversityAppWidget size={size} stats={MOCK_STATS} />
      ...
      <section className="mb-10">
        ...
        <p className="text-sm text-gray-500 mb-5">Install DIY University as a native standalone app</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <InstallCard icon="🍎" title="iOS — Home Screen" accent="#7C3AED" steps={[
            'Open DIY University in Safari',
            'Tap the Share button (square with arrow)',
            'Select "Add to Home Screen"',
            'Tap "Add" — widget appears instantly',
          ]} />
          <InstallCard icon="🤖" title="Android — Home Screen" accent="#10B981" steps={[
            'Open DIY University in Chrome',
            'Tap the menu (three dots) top right',
            'Select "Add to Home Screen"',
            'Confirm — icon added to launcher',
          ]} />
          <InstallCard icon="🖥️" title="Windows Desktop" accent="#D4AF37" steps={[
            'Run create-university-shortcut.ps1',
            'Find "DIY University.lnk" on your desktop',
            'Double-click to open in app mode',
            'Or install via Chrome: click the install icon in the address bar',
          ]} />
        </div>
      </section>

      {/* Shortcut CTA */}
      <div className="rounded-2xl border border-purple-600/25 p-6 flex flex-col md:flex-row md:items-center gap-4"
        style={{ background: 'rgba(124,58,237,0.07)' }}>
        <div className="text-4xl flex-shrink-0">🖥️</div>
        <div className="flex-1">
          <h3 className="font-serif text-lg text-white font-bold mb-1">Windows Desktop Shortcut</h3>
          <p className="text-sm text-gray-400">Opens DIY University in standalone Chrome app mode — no browser chrome, full focus.</p>
          <code className="inline-block mt-2 text-xs bg-black/40 border border-white/10 rounded px-3 py-1.5 text-green-400 font-mono">
            powershell -ExecutionPolicy Bypass -File create-university-shortcut.ps1
          </code>
        </div>
        <Link to="/university" className="flex items-center gap-2 px-5 py-3 rounded-xl text-sm font-medium flex-shrink-0 border border-purple-500/30 text-purple-300 hover:bg-purple-500/10 transition-colors">
          <FiDownload size={14} /> Back to University
        </Link>
      </div>
    </div>
  );
